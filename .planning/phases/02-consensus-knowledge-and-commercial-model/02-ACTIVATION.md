...
Access, secrets, runtime-candidate generation, another upload, routes,
requests, and effects remain separately gated. This partial target is not Plan
02-99 acceptance.
...
| Future greenfield target | status: **partially provisioned; Stage 2 complete; revised Stage 3A authorized** | The fresh D1/R2 target has exact migration and zero-data evidence. One initial unreachable deployment is authorized only after revised local validation, followed by a mandatory read-only stop; Worker/Access/version/deployment/principal/zero-effect/owner-acceptance evidence is not yet claimed. |
