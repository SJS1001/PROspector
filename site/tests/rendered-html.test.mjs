...
test("build/source smoke identifies the controlled workbench and removes the starter", async () => {
  ...
  // or a seed Market Play. Work Unit D removed the Morning Brief that used to
  // carry this copy, so the shell now identifies the workspace by its task and
  // its capability boundary; the generic-shell guard itself is unchanged.
