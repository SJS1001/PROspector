...
test("D-07/D-13 discovery workspace source preserves Product scope and fails closed", async () => {
  ...
  assert.match(workspace, /from\s+["']\.\.\/operator-preference["']/);
  assert.match(workspace, /readOperatorPreference\(["']prospector\.discovery\.product["']\)/);
  assert.match(workspace, /writeOperatorPreference\(["']prospector\.discovery\.product["']/);
