...
test("C2 admits only the owner, derives discovery authority server-side, and projects suggestions through a signed people cursor", async () => {
  ...
  try {
    ...
    const service = discovery.createPersonDiscoveryService({
      ...
      port: testPort.bindPersonDiscoveryTestPort(async () => { calls += 1; return completed("handler"); }),
    ...
    const body = { action: "start_person_discovery", prospectId: fixture.prospectId, expectedProspectRevision: fixture.prospectRevision, maxCandidates: 1, maxProvenancePerCandidate: 1, idempotencyKey: "person-discovery-handler-start" };
    ...
    assert.equal(payload.people.items.length, 1);
...
function completed(key) { return { kind: "completed", candidates: [{ displayName: `Synthetic ${key}`, roleTitle: "Operations", roleSummary: "Synthetic role summary", provenance: [{ sourceReference: "https://example.invalid/team", excerpt: "Synthetic public role listing", retrievedAt: PERSON_DISCOVERY_NOW }] }] }; }
