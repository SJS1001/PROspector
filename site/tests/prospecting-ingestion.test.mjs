...
async function rejectedScenario(key){
 ...
 return {seed,review,readiness,policy,runSubmission,candidateDraft,prospect,decisionAt};
}
...
test("owner rejection forbids non-material re-entry before the 90-day cooldown expires",async()=>{
 const scenario=await rejectedScenario("aaaaaaaaaaaaaaaaaaaaaaaa");try{const before=await snapshotForbiddenOperationalRows(scenario.seed.fixture.database),runId=await replaceProfileConfiguration(scenario,"aaaaaaaaaaaaaaaaaaaaaaaa",scenario.decisionAt+1),window=await scenario.seed.fixture.database.prepare("SELECT window_upper_inclusive FROM prospecting_runs WHERE id=?").bind(runId).first();await assert.rejects(()=>scenario.runSubmission({runId,at:scenario.decisionAt+3,observedAt:Number(window.window_upper_inclusive),submissionKey:"aaaaaaaaaaaaaaaaaaaaaaaa-1",kind:"context-signal"}),(error)=>error?.code==="prospecting_ingestion_rejected");assert.equal(Number((await scenario.seed.fixture.database.prepare("SELECT COUNT(*) count FROM prospect_reentry_events WHERE prospect_id=?").bind(scenario.prospect.id).first()).count),0);assert.deepEqual(await snapshotForbiddenOperationalRows(scenario.seed.fixture.database),before);}finally{await scenario.seed.fixture.dispose();}
});

test("owner-rejected prospect re-enters after the 90-day cooldown without sourced disproof",async()=>{
 const scenario=await rejectedScenario("bbbbbbbbbbbbbbbbbbbbbbbb");try{const before=await snapshotForbiddenOperationalRows(scenario.seed.fixture.database),expiredAt=scenario.decisionAt+90*86_400_000+1,runId=await replaceProfileConfiguration(scenario,"bbbbbbbbbbbbbbbbbbbbbbbb",expiredAt),window=await scenario.seed.fixture.database.prepare("SELECT window_upper_inclusive FROM prospecting_runs WHERE id=?").bind(runId).first(),completed=await scenario.runSubmission({runId,at:expiredAt+3,observedAt:Number(window.window_upper_inclusive),submissionKey:"bbbbbbbbbbbbbbbbbbbbbbbb-1",kind:"context-signal"}),successor=await scenario.seed.fixture.database.prepare("SELECT id,state,active FROM profile_prospects WHERE assessment_id=?").bind(completed.assessments[0].id).first(),event=await scenario.seed.fixture.database.prepare("SELECT event_kind,signal_id FROM prospect_reentry_events WHERE prospect_id=?").bind(scenario.prospect.id).first();assert.deepEqual({state:successor.state,active:Number(successor.active)},{state:"qualified",active:1});assert.deepEqual(event,{event_kind:"review_due",signal_id:null});assert.deepEqual(await snapshotForbiddenOperationalRows(scenario.seed.fixture.database),before);}finally{await scenario.seed.fixture.dispose();}
});

test("application-validated sourced disproof is bound to the rejected lifecycle, candidate digest, and workspace",async()=>{
 const scenario=await rejectedScenario("cccccccccccccccccccccccc");try{
  const runId=await replaceProfileConfiguration(scenario,"cccccccccccccccccccccccc",scenario.decisionAt+1),window=await scenario.seed.fixture.database.prepare("SELECT window_upper_inclusive FROM prospecting_runs WHERE id=?").bind(runId).first();
  const completed=await scenario.runSubmission({runId,at:scenario.decisionAt+5,observedAt:Number(window.window_upper_inclusive),submissionKey:"cccccccccccccccccccccccc-1",materializer:({signals})=>[{...scenario.candidateDraft,sourcedDisproof:{signalFingerprint:signals[0].fingerprint,validationRule:"owner-rejection-material-evidence/v1"}}]});
  const candidate=await scenario.seed.fixture.database.prepare("SELECT id,candidate_digest,fingerprint FROM prospecting_candidates WHERE run_id=?").bind(runId).first(),event=await scenario.seed.fixture.database.prepare("SELECT event_kind,signal_id FROM prospect_reentry_events WHERE prospect_id=?").bind(scenario.prospect.id).first();
  assert.equal(completed.assessments[0].evaluation.outcome,"Passed");assert.equal(event.event_kind,"hard_gate_disproved");assert.ok(event.signal_id);
  assert.equal(await scenario.policy.readValidatedSourcedDisproofSignalId(scenario.seed.fixture.database,{workspaceId:scenario.seed.workspaceId,profileId:scenario.seed.profileId,candidateId:candidate.id,candidateDigest:"0".repeat(64),fingerprint:candidate.fingerprint,priorProspectId:scenario.prospect.id,priorAssessmentId:scenario.prospect.assessment_id,decisionAt:scenario.decisionAt}),null,"a changed candidate digest cannot validate the marker");
  assert.equal(await scenario.policy.readValidatedSourcedDisproofSignalId(scenario.seed.fixture.database,{workspaceId:"wrong-workspace",profileId:scenario.seed.profileId,candidateId:candidate.id,candidateDigest:candidate.candidate_digest,fingerprint:candidate.fingerprint,priorProspectId:scenario.prospect.id,priorAssessmentId:scenario.prospect.assessment_id,decisionAt:scenario.decisionAt}),null,"a marker cannot cross workspaces");
 }finally{await scenario.seed.fixture.dispose();}
});

test("forged sourced-disproof markers never bypass a rejected Prospect cooldown",async()=>{
 const scenario=await rejectedScenario("dddddddddddddddddddddddd");try{
  const runId=await replaceProfileConfiguration(scenario,"dddddddddddddddddddddddd",scenario.decisionAt+1),window=await scenario.seed.fixture.database.prepare("SELECT window_upper_inclusive FROM prospecting_runs WHERE id=?").bind(runId).first();
  await assert.rejects(()=>scenario.runSubmission({runId,at:scenario.decisionAt+5,observedAt:Number(window.window_upper_inclusive),submissionKey:"dddddddddddddddddddddddd-1",kind:"context-signal"}),(error)=>error?.code==="prospecting_ingestion_rejected");
  const candidate=await scenario.seed.fixture.database.prepare("SELECT id,candidate_digest,fingerprint FROM prospecting_candidates WHERE run_id=?").bind(runId).first(),submission=await scenario.seed.fixture.database.prepare("SELECT id FROM runner_submissions WHERE run_id=?").bind(runId).first(),signal=await scenario.seed.fixture.database.prepare("SELECT ps.id,json_extract(pl.lineage_json,'$.observationFingerprint') fingerprint FROM prospecting_signals ps JOIN prospecting_source_lineage pl ON pl.id=ps.source_lineage_id WHERE ps.run_id=?").bind(runId).first();
  await assert.rejects(()=>scenario.policy.appendSourcedDisproofValidationMarker(scenario.seed.fixture.database,{workspaceId:scenario.seed.workspaceId,profileId:scenario.seed.profileId,candidateId:candidate.id,runId,submissionId:submission.id,selection:{signalFingerprint:signal.fingerprint,validationRule:"owner-rejection-material-evidence/v1"},now:scenario.decisionAt+8}),/source_policy_rejected/i,"a non-material signal cannot become sourced disproof");
  const forged=JSON.stringify({schema:"prospect-reentry-disproof-validation/v1",verdict:"sourced_disproof",validatedBy:"application",validationRule:"owner-rejection-material-evidence/v1",workspaceId:scenario.seed.workspaceId,profileId:scenario.seed.profileId,candidateId:candidate.id,candidateDigest:candidate.candidate_digest,prospectFingerprint:candidate.fingerprint,priorProspectId:scenario.prospect.id,priorAssessmentId:scenario.prospect.assessment_id,decisionAt:scenario.decisionAt,signalId:signal.id,validationDigest:"0".repeat(64)});
  await scenario.seed.fixture.database.prepare("INSERT INTO prospecting_source_lineage (id,workspace_id,run_id,submission_id,source_id,source_url,publisher_identity,underlying_origin_identity,independence_group,source_tier,published_at,occurred_at,retrieved_at,excerpt,lineage_json,lineage_digest,created_at) VALUES ('forged-disproof-marker',?,?,?,NULL,'https://forged.example.invalid/','forged.example.invalid','forged.example.invalid','origin:forged.example.invalid',1,NULL,?,?,?, ?,?,?)").bind(scenario.seed.workspaceId,runId,submission.id,scenario.decisionAt+6,scenario.decisionAt+6,"forged marker",forged,"e".repeat(64),scenario.decisionAt+8).run();
  assert.equal(await scenario.policy.readValidatedSourcedDisproofSignalId(scenario.seed.fixture.database,{workspaceId:scenario.seed.workspaceId,profileId:scenario.seed.profileId,candidateId:candidate.id,candidateDigest:candidate.candidate_digest,fingerprint:candidate.fingerprint,priorProspectId:scenario.prospect.id,priorAssessmentId:scenario.prospect.assessment_id,decisionAt:scenario.decisionAt}),null);
  assert.equal(Number((await scenario.seed.fixture.database.prepare("SELECT COUNT(*) count FROM prospect_reentry_events WHERE prospect_id=?").bind(scenario.prospect.id).first()).count),0);
 }finally{await scenario.seed.fixture.dispose();}
});

test("missing or misbound cooldown fails closed even when a later Material Signal exists",async(t)=>{
 for(const [label,key,mutate] of [
  ["missing","eeeeeeeeeeeeeeeeeeeeeeee",(scenario)=>scenario.seed.fixture.database.prepare("DELETE FROM prospect_cooldowns WHERE prospect_id=? AND workspace_id=?").bind(scenario.prospect.id,scenario.seed.workspaceId).run()],
  ["misbound","ffffffffffffffffffffffff",(scenario)=>scenario.seed.fixture.database.prepare("UPDATE prospect_cooldowns SET ends_at=ends_at+1 WHERE prospect_id=? AND workspace_id=?").bind(scenario.prospect.id,scenario.seed.workspaceId).run()],
 ])await t.test(label,async()=>{const scenario=await rejectedScenario(key);try{await mutate(scenario);const runId=await replaceProfileConfiguration(scenario,key,scenario.decisionAt+1),window=await scenario.seed.fixture.database.prepare("SELECT window_upper_inclusive FROM prospecting_runs WHERE id=?").bind(runId).first();await assert.rejects(()=>scenario.runSubmission({runId,at:scenario.decisionAt+5,observedAt:Number(window.window_upper_inclusive),submissionKey:`${key}-1`}),(error)=>error?.code==="prospecting_ingestion_rejected");assert.equal(Number((await scenario.seed.fixture.database.prepare("SELECT COUNT(*) count FROM prospect_reentry_events WHERE prospect_id=?").bind(scenario.prospect.id).first()).count),0);}finally{await scenario.seed.fixture.dispose();}});
});
