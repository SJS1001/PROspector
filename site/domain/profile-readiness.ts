...
export async function createProfileConfigurationCandidate(database: D1Database, principal: InterviewPrincipal, input: { profileId: string; expectedProfileRevision: number; idempotencyKey: string; now?: number; [key: string]: unknown }) {
  ...
  const keyed = await candidateForKey(database, workspace.id, input.idempotencyKey);
  if (keyed) {
    const operation = keyed.candidate_digest ? await candidateOperation(input.profileId, input.expectedProfileRevision, keyed.candidate_digest) : null;
    if (keyed.command_type !== "profile.configuration.candidate" || keyed.subject_type !== "profile" || keyed.subject_id !== input.profileId || Number(keyed.expected_revision) !== input.expectedProfileRevision || keyed.operation_digest !== operation || !keyed.id || !keyed.candidate_digest || keyed.revision === null || !keyed.status) throw new ProfileReadinessConflictError("Idempotency key was used for another Profile candidate");
    return candidateProjection(keyed as CandidateProjectionRow, "replayed");
  }
  ...
  const prior = await database.prepare("SELECT id, revision, candidate_digest, status FROM profile_configuration_candidates WHERE workspace_id = ? AND profile_id = ? AND candidate_digest = ? LIMIT 1").bind(workspace.id, profile.id, digest).first<CandidateProjectionRow>();
  if (prior) return candidateProjection(prior, "deduplicated");
  const now = input.now ?? Date.now(), commandId = v7(), configurationId = v7(), candidateId = v7(), auditId = v7();
  const operation = await candidateOperation(profile.id, profile.revision, digest);
  ...
  try { const results = await database.batch([
    ...
    if (e instanceof ProfileReadinessConflictError) throw e;
    const replay = await candidateForKey(database, workspace.id, input.idempotencyKey);
    if (replay) {
      if (replay.command_type === "profile.configuration.candidate" && replay.subject_type === "profile" && replay.subject_id === profile.id && Number(replay.expected_revision) === profile.revision && replay.operation_digest === operation && replay.id && replay.candidate_digest && replay.revision !== null && replay.status) return candidateProjection(replay as CandidateProjectionRow, "replayed");
      throw new ProfileReadinessConflictError("Idempotency key was used for another Profile candidate");
    }
    const duplicate = await database.prepare("SELECT id, revision, candidate_digest, status FROM profile_configuration_candidates WHERE workspace_id = ? AND profile_id = ? AND candidate_digest = ? LIMIT 1").bind(workspace.id, profile.id, digest).first<CandidateProjectionRow>();
    if (duplicate) return candidateProjection(duplicate, "deduplicated");
    throw race(e, "Profile candidate");
  }
  return candidateProjection({id:candidateId,revision:1,candidate_digest:digest,status:"candidate"}, "created");
}
...
export async function activateProfileConfiguration(database: D1Database, principal: InterviewPrincipal, input: { candidateId: string; expectedRevision: number; expectedDigest: string; idempotencyKey: string; now?: number }) {
  ...
  const operation=await activationOperation(candidate,input);
  const keyed = await activationForKey(database, workspace.id, input.idempotencyKey);
  if (keyed) {
    if (keyed.command_type !== "profile.configuration.activate" || keyed.subject_type !== "profile_configuration_candidate" || keyed.subject_id !== input.candidateId || Number(keyed.expected_revision) !== input.expectedRevision || keyed.operation_digest !== operation || keyed.activation_operation_digest !== operation || keyed.candidate_digest !== input.expectedDigest || !keyed.configuration_id || !keyed.run_id || !keyed.schedule_id || !keyed.timezone || !keyed.intended_local_time || keyed.utc_offset_minutes === null || keyed.next_run_at === null) throw new ProfileReadinessConflictError("Idempotency key was used for another Profile activation");
    return activationProjection(candidate,keyed.configuration_id,keyed.run_id,keyed as Required<ActivationCommandRow>,"replayed");
  }
  const winner = await activationWinner(database, workspace.id, candidate, input, operation);
  if (winner) return activationProjection(candidate,winner.configuration_id,winner.run_id,winner,"deduplicated");
  ...
  const now=input.now??Date.now(), commandId=v7(), activationId=v7(), auditId=v7(), scheduleId=v7(), runId=v7();
  ...
  try { const results = await database.batch([
    ...
    const replay = await activationForKey(database, workspace.id, input.idempotencyKey);
    if (replay) {
      if (replay.command_type === "profile.configuration.activate" && replay.subject_type === "profile_configuration_candidate" && replay.subject_id === input.candidateId && Number(replay.expected_revision) === input.expectedRevision && replay.operation_digest === operation && replay.activation_operation_digest === operation && replay.candidate_digest === input.expectedDigest && replay.configuration_id && replay.run_id && replay.schedule_id && replay.timezone && replay.intended_local_time && replay.utc_offset_minutes !== null && replay.next_run_at !== null) return activationProjection(candidate,replay.configuration_id,replay.run_id,replay as Required<ActivationCommandRow>,"replayed");
      throw new ProfileReadinessConflictError("Idempotency key was used for another Profile activation");
    }
    const won = await activationWinner(database, workspace.id, candidate, input, operation);
    if (won) return activationProjection(candidate,won.configuration_id,won.run_id,won,"deduplicated");
    throw e instanceof ProfileReadinessConflictError ? e : race(e,"Profile activation");
  }
  return activationProjection(candidate,candidate.configuration_id,runId,{schedule_id:scheduleId,timezone:authority.timezone,intended_local_time:localTime,utc_offset_minutes:slot.utcOffsetMinutes,next_run_at:slot.nextRunAt},"created");
}
...
async function activationWinner(database:D1Database, workspaceId:string, candidate:any, input:{expectedRevision:number;expectedDigest:string}, operation:string) {
  return database.prepare("SELECT a.configuration_id,r.id run_id,s.id schedule_id,s.timezone,s.intended_local_time,s.utc_offset_minutes,s.next_run_at FROM profile_configuration_activations a JOIN profile_configuration_candidates c ON c.id=a.candidate_id AND c.workspace_id=a.workspace_id JOIN prospecting_runs r ON r.authority_command_id=a.authority_command_id AND r.configuration_id=a.configuration_id AND r.trigger_kind='initial' JOIN prospecting_schedules s ON s.authority_command_id=a.authority_command_id AND s.configuration_id=a.configuration_id WHERE a.workspace_id=? AND a.candidate_id=? AND c.status='activated' AND c.revision=? AND c.candidate_digest=? AND a.configuration_id=? AND a.operation_digest=? LIMIT 1").bind(workspaceId,candidate.id,input.expectedRevision + 1,input.expectedDigest,candidate.configuration_id,operation).first<any>();
}
...
type MutationDisposition = "created" | "replayed" | "deduplicated";
type CandidateStatus = "candidate" | "activated" | "superseded" | "cancelled";
type CandidateProjectionRow = {id:string;revision:number;candidate_digest:string;status:CandidateStatus};
type CandidateCommandRow = {id:string|null;revision:number|null;candidate_digest:string|null;status:CandidateStatus|null;command_type:string;operation_digest:string;expected_revision:number;subject_type:string;subject_id:string};
type ActivationCommandRow = {configuration_id:string|null;run_id:string|null;schedule_id:string|null;timezone:string|null;intended_local_time:string|null;utc_offset_minutes:number|null;next_run_at:number|null;candidate_digest:string|null;command_type:string;operation_digest:string;activation_operation_digest:string|null;expected_revision:number;subject_type:string;subject_id:string};
async function candidateForKey(d:D1Database,w:string,k:string){return d.prepare("SELECT pc.id,pc.revision,pc.candidate_digest,pc.status,ac.command_type,ac.operation_digest,ac.expected_revision,ac.subject_type,ac.subject_id FROM authority_commands ac LEFT JOIN profile_configuration_candidates pc ON pc.authority_command_id=ac.id AND pc.workspace_id=ac.workspace_id WHERE ac.workspace_id=? AND ac.idempotency_key=? LIMIT 1").bind(w,k).first<CandidateCommandRow>()}
async function activationForKey(d:D1Database,w:string,k:string){return d.prepare("SELECT a.configuration_id,r.id run_id,s.id schedule_id,s.timezone,s.intended_local_time,s.utc_offset_minutes,s.next_run_at,c.candidate_digest,ac.command_type,ac.operation_digest,a.operation_digest activation_operation_digest,ac.expected_revision,ac.subject_type,ac.subject_id FROM authority_commands ac LEFT JOIN profile_configuration_activations a ON a.authority_command_id=ac.id AND a.workspace_id=ac.workspace_id LEFT JOIN profile_configuration_candidates c ON c.id=a.candidate_id AND c.workspace_id=a.workspace_id LEFT JOIN prospecting_runs r ON r.authority_command_id=ac.id AND r.configuration_id=a.configuration_id AND r.trigger_kind='initial' LEFT JOIN prospecting_schedules s ON s.authority_command_id=ac.id AND s.configuration_id=a.configuration_id WHERE ac.workspace_id=? AND ac.idempotency_key=? LIMIT 1").bind(w,k).first<ActivationCommandRow>()}
async function candidateOperation(profileId:string,expectedRevision:number,digest:string){return sha256(stable({action:"profile.configuration.candidate",profileId,expectedRevision,digest}))}
async function activationOperation(candidate:any,input:{candidateId:string;expectedRevision:number;expectedDigest:string}){const manifest=parseManifest(candidate.configuration_manifest_json);const profile=manifest.profile;if(!profile||typeof profile!=="object"||Array.isArray(profile)||!Number.isInteger((profile as Record<string,unknown>).revision))throw new ProfileReadinessConflictError("Profile configuration candidate has invalid frozen authority");return sha256(stable({action:"profile.configuration.activate",candidateId:input.candidateId,digest:input.expectedDigest,profileRevision:(profile as Record<string,unknown>).revision}))}
function mutationResult(disposition:MutationDisposition){return{disposition,replayed:disposition==="replayed",deduplicated:disposition==="deduplicated"}as const}
function candidateProjection(r:CandidateProjectionRow,disposition:MutationDisposition){const status=r.status==="candidate"?"candidate_not_active":r.status;return{id:r.id,revision:Number(r.revision),digest:r.candidate_digest,status,immutable:true,...mutationResult(disposition)}}
function activationProjection(c:any,configurationId:string,runId:string,schedule:{schedule_id:string;timezone:string;intended_local_time:string;utc_offset_minutes:number;next_run_at:number},disposition:MutationDisposition){return{configuration:{id:configurationId,digest:c.candidate_digest,active:true,immutable:true},initialRun:{id:runId,trigger:"initial",executionState:"blocked_missing_capability"},schedule:{id:schedule.schedule_id,timezone:schedule.timezone,localTime:schedule.intended_local_time,utcOffsetMinutes:Number(schedule.utc_offset_minutes),nextRunAt:Number(schedule.next_run_at),cadence:"weekdays",executionState:"blocked_missing_capability"},...mutationResult(disposition)}}
