...
export async function createProfileConfigurationCandidate(database: D1Database, principal: InterviewPrincipal, input: { profileId: string; expectedProfileRevision: number; idempotencyKey: string; now?: number; [key: string]: unknown }) {
  ...
  if (keyed) {
    ...
    if (keyed.command_type !== "profile.configuration.candidate" || keyed.subject_type !== "profile" || keyed.subject_id !== input.profileId || Number(keyed.expected_revision) !== input.expectedProfileRevision || keyed.operation_digest !== operation || !keyed.id || !keyed.candidate_digest || keyed.revision === null) throw new ProfileReadinessConflictError("Idempotency key was used for another Profile candidate");
    return candidateProjection(keyed as Required<CandidateCommandRow>, "replayed");
  }
  ...
  try { const results = await database.batch([
    ...
    if (replay) {
      if (replay.command_type === "profile.configuration.candidate" && replay.subject_type === "profile" && replay.subject_id === profile.id && Number(replay.expected_revision) === profile.revision && replay.operation_digest === operation && replay.id && replay.candidate_digest && replay.revision !== null) return candidateProjection(replay as Required<CandidateCommandRow>, "replayed");
      throw new ProfileReadinessConflictError("Idempotency key was used for another Profile candidate");
    }
    const duplicate = await database.prepare("SELECT id, revision, candidate_digest FROM profile_configuration_candidates WHERE workspace_id = ? AND profile_id = ? AND candidate_digest = ? LIMIT 1").bind(workspace.id, profile.id, digest).first<{id:string;revision:number;candidate_digest:string}>();
    if (duplicate) return candidateProjection(duplicate, "deduplicated");
    throw race(e, "Profile candidate");
  }
  return candidateProjection({id:candidateId,revision:1,candidate_digest:digest}, "created");
}
...
type CandidateCommandRow = {id:string|null;revision:number|null;candidate_digest:string|null;command_type:string;operation_digest:string;expected_revision:number;subject_type:string;subject_id:string};
...
async function candidateForKey(d:D1Database,w:string,k:string){return d.prepare("SELECT pc.id,pc.revision,pc.candidate_digest,ac.command_type,ac.operation_digest,ac.expected_revision,ac.subject_type,ac.subject_id FROM authority_commands ac LEFT JOIN profile_configuration_candidates pc ON pc.authority_command_id=ac.id AND pc.workspace_id=ac.workspace_id WHERE ac.workspace_id=? AND ac.idempotency_key=? LIMIT 1").bind(w,k).first<CandidateCommandRow>()}
...
function candidateProjection(r:{id:string;revision:number;candidate_digest:string},disposition:MutationDisposition){return{id:r.id,revision:Number(r.revision),digest:r.candidate_digest,status:"candidate_not_active" as const,immutable:true,...mutationResult(disposition)}}
