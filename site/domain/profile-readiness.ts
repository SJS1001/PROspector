...
import { v7 } from "uuid";
import { profileSlotKey } from "./prospecting-schedule";
...
export function evaluateProfileReadiness(input: { profile: { id: string; lifecycle: string }; authority?: Partial<Authority>; versions?: Version[] }) {
  ...
  const prerequisites = !input.authority?.productConfiguration ? ["product_configuration"] : !input.authority?.acceptedPlay ? ["accepted_play"] : !input.authority?.offer ? ["confirmed_offer_lineage"] : [];
...
export async function createProfileConfigurationCandidate(database: D1Database, principal: InterviewPrincipal, input: { profileId: string; expectedProfileRevision: number; idempotencyKey: string; now?: number; [key: string]: unknown }) {
  ...
  ]); } catch (e) { throw race(e, "Profile candidate") }
...
export async function activateProfileConfiguration(database: D1Database, principal: InterviewPrincipal, input: { candidateId: string; expectedRevision: number; expectedDigest: string; idempotencyKey: string; now?: number }) {
  ...
  try { const results = await database.batch([
    ...
    if (won) return activationProjection(candidate,won.configuration_id,won.run_id,won.schedule_id);
    throw e instanceof ProfileReadinessConflictError ? e : race(e,"Profile activation");
  }
  return activationProjection(candidate,candidate.configuration_id,runId,scheduleId);
}
...
async function resolveAuthority(database:D1Database, workspaceId:string, profile:Profile, required:boolean):Promise<Authority|null> {
  const product=await database.prepare("SELECT id,digest,manifest_json FROM typed_configurations WHERE workspace_id=? AND owner_type='product' AND owner_id=? AND kind='product_discovery' AND active=1 LIMIT 1").bind(workspaceId,profile.product_id).first<{id:string;digest:string;manifest_json:string}>();
  ...
  const authority = product && validDigest(product.digest) && productManifest && sourcePolicy && runnerPolicy && scheduleSemantics && replacementDirectives && offer && validDigest(offer.digest) && play&&["active","ready"].includes(play.lifecycle) && ["draft","ready"].includes(profile.lifecycle) && timezone?.timezone
  ...
  if(required && !authority) { if(!product||!productManifest) throw new ProfileReadinessConflictError("Product Discovery Configuration authority is unavailable"); if(!play||!["active","ready"].includes(play.lifecycle)) throw new ProfileReadinessConflictError("Accepted active Market Play authority is unavailable"); if(!offer) throw new ProfileReadinessConflictError("Confirmed Offer lineage authority is unavailable"); if(!sourcePolicy||!runnerPolicy||!scheduleSemantics||!replacementDirectives) throw new ProfileReadinessConflictError("Pinned source, runner, schedule, and replacement authority is unavailable"); throw new ProfileReadinessConflictError("Profile authority is unavailable"); }
  return authority;
}
