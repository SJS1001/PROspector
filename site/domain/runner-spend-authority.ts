...
export async function deriveRunnerOperationKey(authority: Pick<RunnerSpendAuthority, "principalSubject" | "grant">): Promise<string> {
  const { grant } = authority;
  return `ro_${await digest(stable({ ownerSubject: authority.principalSubject, grantId: grant.id, providerId: grant.providerId, model: grant.model, catalogRef: grant.catalogRef, runType: grant.runType, scopeId: grant.scopeId, perRunCostMinor: grant.perRunCostMinor, monthlyCostMinor: grant.monthlyCostMinor, currency: grant.currency, expiresAt: grant.expiresAt, maxRetries: grant.maxRetries }))}`;
}
