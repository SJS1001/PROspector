...
export async function createReplacementCandidate(database: D1Database, principal: InterviewPrincipal, input: CandidateInput) {
  ...
  const activeDependencies = await database.prepare("SELECT knowledge_version_id FROM configuration_knowledge_dependencies WHERE configuration_id = ? ORDER BY knowledge_version_id").bind(active.id).all<{ knowledge_version_id: string }>();
  ...
  try {
    await database.batch([
      ...
      ...[...new Set(activeDependencies.results.map((dependency) => dependency.knowledge_version_id === current.id ? proposed.id : dependency.knowledge_version_id))].sort().map((knowledgeVersionId) => database.prepare("INSERT INTO configuration_knowledge_dependencies (configuration_id, knowledge_version_id, created_at) VALUES (?, ?, ?)").bind(configurationId, knowledgeVersionId, now)),
