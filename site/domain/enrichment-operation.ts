...
export async function executeEnrichmentOperation(repository: EnrichmentAuthorityRepository, port: unknown, input: { reservationId: string; now: number }): Promise<ExecuteEnrichmentResult> {
  ...
  const observations = outcome.kind === "rejected" ? [] : ingestEvidence(assignment.evidenceAssignments, outcome.evidence);
...
function validAssignment(value: unknown, reservationId: string, now: number): value is AuthorizedEnrichmentAssignment {
  ...
  return validAssignmentBindings(value.evidenceAssignments, value.workspaceId, value.configurationId, value.configurationDigest, value.prospectIds);
}
...
function validAssignmentBindings(value: unknown, workspaceId: string, configurationId: string, configurationDigest: string, prospectIds: readonly string[]): value is readonly AssignedContactEvidence[] {
  if (!Array.isArray(value) || value.length !== prospectIds.length || new Set(value.map((item) => plain(item) ? item.prospectId : null)).size !== prospectIds.length) return false;
  return value.every((item) => plain(item) && bounded(item.prospectId, 256) && prospectIds.includes(item.prospectId) && item.workspaceId === workspaceId && bounded(item.contactId, 256) && item.profileConfigurationId === configurationId && item.profileConfigurationDigest === configurationDigest);
}
...
function ingestEvidence(bindings: readonly AssignedContactEvidence[], envelopes: readonly unknown[]): readonly ContactObservation[] | null {
  const observations: ContactObservation[] = [];
  for (const envelope of envelopes) {
    if (!plain(envelope) || !bounded(envelope.prospectId, 256)) return null;
    const binding = bindings.find((item) => item.prospectId === envelope.prospectId && item.workspaceId === envelope.workspaceId && item.contactId === envelope.contactId && item.profileConfigurationId === envelope.profileConfigurationId && item.profileConfigurationDigest === envelope.profileConfigurationDigest);
    if (!binding) return null;
    const result = ingestContactEvidence(binding as ContactEvidenceAssignment, envelope);
