...
async function readProjection(request: Request, dependencies: PersonDiscoveryHandlerDependencies, workspaceId: string, subject: string) {
  ...
  const approvedRows = (await dependencies.database.prepare(`SELECT p.id,p.revision,p.updated_at
  ...
  ) ORDER BY p.updated_at DESC,p.id DESC LIMIT 100`).bind(workspaceId, subject).all<{ id: string; revision: number; updated_at: number }>()).results;
  const approved = await Promise.all(approvedRows.filter((row) => opaque(row.id) && positive(row.revision) && positive(Number(row.updated_at))).map(async (row) => Object.freeze({ prospectId: row.id, prospectRevision: Number(row.revision), label: prospectLabel(b64(await sign(`prospect-label:${row.id}`, dependencies.subjectPepper, workspaceId, subject)).slice(0, 12), Number(row.updated_at)), knownPerson: await currentKnownPerson(dependencies.database, { workspaceId, principalSubject: subject }, row.id) })));
  const prospectId = url.searchParams.get("prospectId");
  if (!prospectId) return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: Object.freeze([]), history: emptyHistory(), people: emptyPeople() });
  ...
  if (!authority) {
    ...
    return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: Object.freeze([]), history: emptyHistory(), people: emptyPeople() });
  }
  ...
  if (!latest || !opaque(latest.id) || !["requested", "completed", "needs_reconciliation"].includes(latest.state) || !positive(latest.created_at)) {
    if (cursor) throw new CursorDriftError();
    return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId, dependencies.subjectPepper, subject), history: await history(dependencies.database, workspaceId, subject, dependencies.subjectPepper, prospectId, projectionNow), people: emptyPeople() });
  }
  ...
  if (!runCurrent) return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId, dependencies.subjectPepper, subject), history: await history(dependencies.database, workspaceId, subject, dependencies.subjectPepper, prospectId, projectionNow), people: Object.freeze({ runId: null, status: "stale_authority", items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  if (latest.state !== "completed") return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId, dependencies.subjectPepper, subject), history: await history(dependencies.database, workspaceId, subject, dependencies.subjectPepper, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, resultDigest: digest(latest.result_digest) ? latest.result_digest : null, status: latest.state, items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  if (!digest(latest.result_digest)) { if (cursor) throw new CursorDriftError(); return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId, dependencies.subjectPepper, subject), history: await history(dependencies.database, workspaceId, subject, dependencies.subjectPepper, prospectId, projectionNow), people: emptyPeople() }); }
  const top = cursor?.highWater ?? await candidateTop(dependencies.database, workspaceId, latest.id);
  if (!top) return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId, dependencies.subjectPepper, subject), history: await history(dependencies.database, workspaceId, subject, dependencies.subjectPepper, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, resultDigest: latest.result_digest, status: "completed", items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  const after = cursor?.after;
  const whereAfter = after ? " AND (candidate.created_at<? OR (candidate.created_at=? AND candidate.id<?))" : "";
  const bind: unknown[] = [workspaceId, latest.id, top.time, top.time, top.id];
  if (after) bind.push(after.time, after.time, after.id);
  const rows = (await dependencies.database.prepare(`SELECT candidate.id,candidate.ordinal,candidate.display_name,candidate.role_title,candidate.role_summary,candidate.candidate_digest,candidate.created_at,candidate.payload_expires_at,candidate.redacted_at, provenance.source_reference,provenance.excerpt,provenance.retrieved_at FROM person_discovery_candidates candidate LEFT JOIN person_discovery_provenance provenance ON provenance.id=(SELECT p.id FROM person_discovery_provenance p WHERE p.workspace_id=candidate.workspace_id AND p.candidate_id=candidate.id AND p.redacted_at IS NULL AND p.payload_expires_at>? ORDER BY p.ordinal,p.id LIMIT 1) WHERE candidate.workspace_id=? AND candidate.run_id=? AND (candidate.created_at<? OR (candidate.created_at=? AND candidate.id<=?))${whereAfter} ORDER BY candidate.created_at DESC,candidate.id DESC LIMIT ${PERSON_DISCOVERY_PAGE_LIMIT + 1}`).bind(projectionNow, ...bind).all<{ id: string; ordinal: number; display_name: string; role_title: string; role_summary: string; candidate_digest: string; created_at: number; payload_expires_at: number; redacted_at: number | null; source_reference: string | null; excerpt: string | null; retrieved_at: number | null }>()).results;
  const now = projectionNow;
  const returned = rows.slice(0, PERSON_DISCOVERY_PAGE_LIMIT).map((row) => Number(row.redacted_at) > 0 || Number(row.payload_expires_at) <= now ? Object.freeze({ candidateId: row.id, ordinal: Number(row.ordinal), state: "payload_unavailable" as const, eligible: false }) : Object.freeze({ candidateId: row.id, ordinal: Number(row.ordinal), displayName: row.display_name, roleTitle: row.role_title, roleSummary: row.role_summary, candidateDigest: row.candidate_digest, provenance: boundedText(row.source_reference, 1000) && boundedText(row.excerpt, 1000) && positive(Number(row.retrieved_at)) ? Object.freeze({ sourceReference: row.source_reference, excerpt: row.excerpt, retrievedAt: Number(row.retrieved_at) }) : null, state: "suggestion_not_contact" as const, eligible: false }));
  ...
  return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId, dependencies.subjectPepper, subject), history: await history(dependencies.database, workspaceId, subject, dependencies.subjectPepper, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, resultDigest: latest.result_digest, status: "completed", items: returned, pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: returned.length, hasNext: rows.length > PERSON_DISCOVERY_PAGE_LIMIT, nextCursor }) }) });
}
...
async function currentKnownPerson(database: D1Database, scope: { workspaceId: string; principalSubject: string }, prospectId: string) {
  const row = await database.prepare(`SELECT EXISTS(
    ...
    WHERE relevance.workspace_id=? AND relevance.prospect_id=? AND decision.contact_id=relevance.contact_id AND run.id=(SELECT latest.id FROM person_discovery_runs latest WHERE latest.workspace_id=run.workspace_id AND latest.prospect_id=run.prospect_id ORDER BY latest.created_at DESC,latest.id DESC LIMIT 1)
...
async function linkableContacts(database: D1Database, workspaceId: string, secret: string, subject: string) {
  const rows = (await database.prepare(`SELECT id,revision,display_name FROM contacts WHERE workspace_id=? ORDER BY updated_at DESC,id DESC LIMIT 20`).bind(workspaceId).all<{ id: string; revision: number; display_name: string }>()).results;
  return Object.freeze(await Promise.all(rows.filter((row) => opaque(row.id) && positive(Number(row.revision)) && boundedText(row.display_name, 160)).map(async (row) => Object.freeze({ contactId: row.id, contactRevision: Number(row.revision), label: contactLabel(row.display_name, b64(await sign(`contact-label:${row.id}`, secret, workspaceId, subject)).slice(0, 12)) }))));
}

async function history(database: D1Database, workspaceId: string, subject: string, secret: string, prospectId: string, now = Date.now()) {
  const bind = [subject, workspaceId, prospectId];
  const [runs, decisions, relevance, intents] = await Promise.all([
    database.prepare(`${currentAuthorityCte()} SELECT run.id,run.prospect_id,event.state,event.result_digest FROM current_authority JOIN person_discovery_runs run ON run.id=current_authority.run_id AND run.workspace_id=current_authority.workspace_id JOIN person_discovery_run_events event ON event.run_id=run.id AND event.durable_revision=(SELECT max(x.durable_revision) FROM person_discovery_run_events x WHERE x.run_id=run.id) ORDER BY run.created_at DESC,run.id DESC LIMIT 50`).bind(...bind).all<{ id: string; prospect_id: string; state: string; result_digest: string | null }>(),
    database.prepare(`${currentAuthorityCte()} SELECT decision.id,decision.run_id,current_authority.prospect_id,decision.decision,decision.candidate_id,decision.contact_id FROM current_authority JOIN person_discovery_owner_decisions decision ON decision.run_id=current_authority.run_id AND decision.workspace_id=current_authority.workspace_id ORDER BY decision.created_at DESC,decision.id DESC LIMIT 50`).bind(...bind).all<{ id: string; run_id: string; prospect_id: string; decision: string; candidate_id: string | null; contact_id: string | null }>(),
    database.prepare(`${currentAuthorityCte()} SELECT relevance.id,relevance.prospect_id,relevance.contact_id,relevance.decision_id,relevance.role_title,contact.revision contact_revision,contact.display_name FROM current_authority JOIN prospect_contact_role_relevance relevance ON relevance.prospect_id=current_authority.prospect_id AND relevance.workspace_id=current_authority.workspace_id JOIN person_discovery_owner_decisions decision ON decision.id=relevance.decision_id AND decision.run_id=current_authority.run_id AND decision.contact_id=relevance.contact_id JOIN contacts contact ON contact.id=relevance.contact_id AND contact.workspace_id=relevance.workspace_id ORDER BY relevance.created_at DESC,relevance.id DESC LIMIT 50`).bind(...bind).all<{ id: string; prospect_id: string; contact_id: string; decision_id: string; role_title: string; contact_revision: number; display_name: string }>(),
    database.prepare(`${currentAuthorityCte()} SELECT intent.id,intent.relevance_id,intent.intent,intent.channel,intent.source_observation_id,'intent_only' effect FROM current_authority JOIN prospect_contact_role_relevance relevance ON relevance.prospect_id=current_authority.prospect_id AND relevance.workspace_id=current_authority.workspace_id JOIN person_discovery_owner_decisions decision ON decision.id=relevance.decision_id AND decision.run_id=current_authority.run_id AND decision.contact_id=relevance.contact_id JOIN contact_verification_intents intent ON intent.relevance_id=relevance.id AND intent.workspace_id=relevance.workspace_id ORDER BY intent.created_at DESC,intent.id DESC LIMIT 50`).bind(...bind).all<{ id: string; relevance_id: string; intent: string; channel: string; source_observation_id: string | null; effect: string }>(),
  ]);
  const projectedRelevance = await Promise.all(relevance.results.filter((row) => opaque(row.id) && opaque(row.prospect_id) && opaque(row.contact_id) && opaque(row.decision_id) && positive(Number(row.contact_revision)) && boundedText(row.display_name, 160) && boundedText(row.role_title, 160)).map(async (row) => Object.freeze({ relevanceId: row.id, prospectId: row.prospect_id, contactId: row.contact_id, contactRevision: Number(row.contact_revision), contactLabel: contactLabel(row.display_name, b64(await sign(`contact-label:${row.contact_id}`, secret, workspaceId, subject)).slice(0, 12)), decisionId: row.decision_id, roleTitle: row.role_title, current: true as const, verificationChannels: Object.freeze(["email", "phone"] as const) })));
  return Object.freeze({
    ...
    decisions: decisions.results.filter((row) => opaque(row.id) && opaque(row.run_id) && opaque(row.prospect_id) && ["no_match", "create_new", "link_existing"].includes(row.decision)).map((row) => Object.freeze({ decisionId: row.id, runId: row.run_id, prospectId: row.prospect_id, decision: row.decision, candidateId: opaque(row.candidate_id) ? row.candidate_id : null, contactId: opaque(row.contact_id) ? row.contact_id : null })),
    relevance: Object.freeze(projectedRelevance),
    verificationIntents: intents.results.filter((row) => opaque(row.id) && opaque(row.relevance_id) && ["initial_verification", "stale_refresh"].includes(row.intent) && ["email", "phone"].includes(row.channel) && row.effect === "intent_only" && (row.intent === "initial_verification" ? row.source_observation_id === null : opaque(row.source_observation_id))).map((row) => Object.freeze({ intentId: row.id, relevanceId: row.relevance_id, intent: row.intent, channel: row.channel, sourceObservationId: row.source_observation_id, effect: "intent_only" as const })),
    staleTrustedObservations: await staleTrustedObservations(database, workspaceId, subject, prospectId, now),
  });
}

async function staleTrustedObservations(database: D1Database, workspaceId: string, subject: string, prospectId: string, now: number) {
  const rows = (await database.prepare(`${currentAuthorityCte()} SELECT observation.id,relevance.id relevance_id,observation.kind,observation.verified_at
    FROM current_authority
    JOIN prospect_contact_role_relevance relevance ON relevance.prospect_id=current_authority.prospect_id AND relevance.workspace_id=current_authority.workspace_id
    JOIN person_discovery_owner_decisions decision ON decision.id=relevance.decision_id AND decision.run_id=current_authority.run_id AND decision.contact_id=relevance.contact_id
    JOIN typed_configurations cfg ON cfg.id=current_authority.configuration_id AND cfg.workspace_id=current_authority.workspace_id AND cfg.digest=current_authority.configuration_digest AND cfg.revision=current_authority.configuration_revision AND cfg.active=1
    JOIN contact_point_observations observation ON observation.workspace_id=relevance.workspace_id AND observation.contact_id=relevance.contact_id AND observation.configuration_id=cfg.id AND observation.configuration_digest=cfg.digest AND observation.kind IN ('email','phone') AND observation.verification_class IN ('mailbox_verified','source_verified') AND observation.verified_at IS NOT NULL
    WHERE ? >= observation.verified_at + CASE WHEN observation.kind='phone' THEN json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.verifiedBusinessPhoneFreshnessMs') WHEN observation.verification_class='mailbox_verified' THEN json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.mailboxVerifiedEmailFreshnessMs') ELSE json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.sourceVerifiedEmailFreshnessMs') END
    AND NOT EXISTS (SELECT 1 FROM contact_point_observations newer WHERE newer.workspace_id=observation.workspace_id AND newer.contact_id=observation.contact_id AND newer.configuration_id=observation.configuration_id AND newer.configuration_digest=observation.configuration_digest AND newer.kind=observation.kind AND newer.verification_class IN ('mailbox_verified','source_verified') AND newer.verified_at IS NOT NULL AND newer.verified_at>observation.verified_at AND ? < newer.verified_at + CASE WHEN newer.kind='phone' THEN json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.verifiedBusinessPhoneFreshnessMs') WHEN newer.verification_class='mailbox_verified' THEN json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.mailboxVerifiedEmailFreshnessMs') ELSE json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.sourceVerifiedEmailFreshnessMs') END)
    ORDER BY observation.verified_at DESC,observation.id DESC LIMIT 20`).bind(subject, workspaceId, prospectId, now, now).all<{ id: string; relevance_id: string; kind: string; verified_at: number }>()).results;
  return Object.freeze(rows.filter((row) => opaque(row.id) && opaque(row.relevance_id) && (row.kind === "email" || row.kind === "phone") && positive(Number(row.verified_at))).map((row) => Object.freeze({ sourceObservationId: row.id, relevanceId: row.relevance_id, channel: row.kind, verifiedAt: Number(row.verified_at), status: "stale" as const })));
}

function currentAuthorityCte() {
  return `WITH current_authority AS (
    SELECT run.id run_id,run.workspace_id,run.prospect_id,run.configuration_id,run.configuration_digest,run.configuration_revision
    FROM person_discovery_runs run
    JOIN workspaces workspace ON workspace.id=run.workspace_id AND workspace.owner_subject=? AND workspace.revision=run.workspace_revision
    JOIN workspace_companies wc ON wc.workspace_id=workspace.id
    JOIN companies company ON company.id=wc.company_id AND company.workspace_id=workspace.id AND company.status='active'
    JOIN profile_prospects prospect ON prospect.id=run.prospect_id AND prospect.workspace_id=workspace.id AND prospect.profile_id=run.profile_id AND prospect.active=1 AND prospect.state='approved' AND prospect.revision=run.prospect_revision
    JOIN customer_profiles profile ON profile.id=prospect.profile_id AND profile.workspace_id=workspace.id AND profile.lifecycle='ready'
    JOIN market_plays play ON play.id=profile.play_id AND play.workspace_id=workspace.id AND play.lifecycle='active'
    JOIN products product ON product.id=play.product_id AND product.workspace_id=workspace.id AND product.company_id=company.id AND product.lifecycle='ready'
    JOIN typed_configurations cfg ON cfg.id=run.configuration_id AND cfg.workspace_id=workspace.id AND cfg.owner_type='profile' AND cfg.owner_id=profile.id AND cfg.kind='profile_effective' AND cfg.active=1 AND cfg.digest=run.configuration_digest AND cfg.revision=run.configuration_revision
    JOIN prospecting_candidates source_candidate ON source_candidate.id=prospect.candidate_id AND source_candidate.workspace_id=workspace.id AND source_candidate.profile_id=profile.id AND source_candidate.configuration_id=cfg.id AND source_candidate.status IN ('observed','qualified')
    JOIN qualification_assessments assessment ON assessment.id=prospect.assessment_id AND assessment.workspace_id=workspace.id AND assessment.candidate_id=source_candidate.id AND assessment.configuration_id=cfg.id AND assessment.configuration_digest=cfg.digest AND assessment.outcome='Passed'
    JOIN prospect_review_decisions review ON review.prospect_id=prospect.id AND review.workspace_id=workspace.id AND review.assessment_id=assessment.id AND review.decision='approve'
    WHERE run.workspace_id=? AND run.prospect_id=? AND run.id=(SELECT latest.id FROM person_discovery_runs latest WHERE latest.workspace_id=run.workspace_id AND latest.prospect_id=run.prospect_id ORDER BY latest.created_at DESC,latest.id DESC LIMIT 1)
  )`;
}

function emptyHistory() { return Object.freeze({ runs: Object.freeze([]), decisions: Object.freeze([]), relevance: Object.freeze([]), verificationIntents: Object.freeze([]), staleTrustedObservations: Object.freeze([]) }); }
function prospectLabel(discriminator: string, updatedAt: number) { return `Approved prospect · ${discriminator} · reviewed ${new Date(updatedAt).toISOString().slice(0, 10)}`.slice(0, 160); }
function contactLabel(displayName: string, discriminator: string) { return `${displayName.slice(0, 145)} · ${discriminator}`; }
