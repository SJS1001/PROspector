...
async function readProjection(request: Request, dependencies: PersonDiscoveryHandlerDependencies, workspaceId: string, subject: string) {
  ...
  const approved = await Promise.all(approvedRows.filter((row) => opaque(row.id) && positive(row.revision) && positive(Number(row.updated_at))).map(async (row) => Object.freeze({ prospectId: row.id, prospectRevision: Number(row.revision), label: `Approved prospect reviewed ${new Date(Number(row.updated_at)).toISOString().slice(0, 10)}`, knownPerson: await currentKnownPerson(dependencies.database, { workspaceId, principalSubject: subject }, row.id) })));
  const prospectId = url.searchParams.get("prospectId");
  if (!prospectId) return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, undefined, projectionNow), people: emptyPeople() });
  ...
  if (!authority) {
    ...
    return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, undefined, projectionNow), people: emptyPeople() });
  }
  ...
  if (!latest || !opaque(latest.id) || !["requested", "completed", "needs_reconciliation"].includes(latest.state) || !positive(latest.created_at)) {
    if (cursor) throw new CursorDriftError();
    return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, prospectId, projectionNow), people: emptyPeople() });
  }
  ...
  if (!runCurrent) return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, prospectId, projectionNow), people: Object.freeze({ runId: null, status: "stale_authority", items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  if (latest.state !== "completed") return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, status: latest.state, items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  if (!digest(latest.result_digest)) { if (cursor) throw new CursorDriftError(); return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, prospectId, projectionNow), people: emptyPeople() }); }
  const top = cursor?.highWater ?? await candidateTop(dependencies.database, workspaceId, latest.id);
  if (!top) return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, resultDigest: latest.result_digest, status: "completed", items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  ...
  return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, status: "completed", items: returned, pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: returned.length, hasNext: rows.length > PERSON_DISCOVERY_PAGE_LIMIT, nextCursor }) }) });
}
...
async function history(database: D1Database, workspaceId: string, prospectId?: string, now = Date.now()) {
  ...
  return Object.freeze({
    ...
    relevance: relevance.results.filter((row) => opaque(row.id) && opaque(row.prospect_id) && opaque(row.contact_id) && opaque(row.decision_id) && positive(Number(row.contact_revision)) && boundedText(row.display_name, 160)).map((row) => Object.freeze({ relevanceId: row.id, prospectId: row.prospect_id, contactId: row.contact_id, contactRevision: Number(row.contact_revision), contactLabel: row.display_name, decisionId: row.decision_id, roleTitle: row.role_title })),
    ...
    staleTrustedObservations: await staleTrustedObservations(database, workspaceId, prospectId, now),
  });
}

async function staleTrustedObservations(database: D1Database, workspaceId: string, prospectId: string | undefined, now: number) {
  const prospectFilter = prospectId ? " AND relevance.prospect_id=?" : "";
  const rows = (await database.prepare(`SELECT observation.id,relevance.id relevance_id,observation.kind,observation.verified_at
    FROM prospect_contact_role_relevance relevance
    JOIN person_discovery_owner_decisions decision ON decision.id=relevance.decision_id AND decision.workspace_id=relevance.workspace_id
    JOIN person_discovery_runs run ON run.id=decision.run_id AND run.workspace_id=decision.workspace_id
    JOIN profile_prospects prospect ON prospect.id=relevance.prospect_id AND prospect.workspace_id=relevance.workspace_id AND prospect.revision=run.prospect_revision
    JOIN typed_configurations cfg ON cfg.id=run.configuration_id AND cfg.workspace_id=run.workspace_id AND cfg.digest=run.configuration_digest AND cfg.revision=run.configuration_revision AND cfg.active=1
    ...
    WHERE relevance.workspace_id=? AND ? >= observation.verified_at + CASE WHEN observation.kind='phone' THEN json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.verifiedBusinessPhoneFreshnessMs') WHEN observation.verification_class='mailbox_verified' THEN json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.mailboxVerifiedEmailFreshnessMs') ELSE json_extract(cfg.manifest_json,'$.confirmedCategoryInputs.contact_strategy[0].value.sourceVerifiedEmailFreshnessMs') END${prospectFilter}
    ...
    ORDER BY observation.verified_at DESC,observation.id DESC LIMIT 20`).bind(workspaceId, now, ...(prospectId ? [prospectId] : []), now).all<{ id: string; relevance_id: string; kind: string; verified_at: number }>()).results;
