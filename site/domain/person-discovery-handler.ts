...
function validRun(value: unknown) { return record(value) && opaque(value.id) && digest(value.requestDigest) && opaque(value.operationKey) && ["requested", "completed", "needs_reconciliation"].includes(String(value.status)) && (value.resultDigest === null || digest(value.resultDigest)) && positive(value.requestedDeadlineAt) && Array.isArray(value.candidates); }
function validDecision(value: unknown) { return record(value) && opaque(value.id) && opaque(value.runId) && digest(value.decisionDigest) && ["no_match", "create_new", "link_existing"].includes(String(value.decision)) && (value.decision === "no_match" ? value.candidateId === null && value.contactId === null && value.relevanceId === null : opaque(value.candidateId) && opaque(value.contactId) && opaque(value.relevanceId)); }
function validIntent(value: unknown) { return record(value) && opaque(value.id) && digest(value.intentDigest) && ["initial_verification", "stale_refresh"].includes(String(value.intent)) && ["email", "phone"].includes(String(value.channel)) && positive(value.freshnessWindowMs) && digest(value.freshnessPolicyDigest) && (value.intent === "initial_verification" ? value.sourceObservationId === null : opaque(value.sourceObservationId)); }
