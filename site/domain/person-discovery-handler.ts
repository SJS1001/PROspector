...
async function readProjection(request: Request, dependencies: PersonDiscoveryHandlerDependencies, workspaceId: string, subject: string) {
  ...
  if (!latest || !opaque(latest.id) || !["requested", "completed", "needs_reconciliation"].includes(latest.state) || !positive(latest.created_at)) {
    if (cursor) throw new CursorDriftError();
    return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, subject, prospectId, projectionNow), people: emptyPeople() });
  }
  ...
  if (!runCurrent) return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, subject, prospectId, projectionNow), people: Object.freeze({ runId: null, status: "stale_authority", items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  if (latest.state !== "completed") return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, subject, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, resultDigest: digest(latest.result_digest) ? latest.result_digest : null, status: latest.state, items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  ...
  if (!top) return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, subject, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, resultDigest: latest.result_digest, status: "completed", items: Object.freeze([]), pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: 0, hasNext: false, nextCursor: null }) }) });
  ...
  return Object.freeze({ capability: dependencies.personDiscoveryService ? "test_composed_only" : "reject_only", approvedProspects: approved, linkableContacts: await linkableContacts(dependencies.database, workspaceId), history: await history(dependencies.database, workspaceId, subject, prospectId, projectionNow), people: Object.freeze({ runId: latest.id, resultDigest: latest.result_digest, status: "completed", items: returned, pageInfo: Object.freeze({ limit: PERSON_DISCOVERY_PAGE_LIMIT, returned: returned.length, hasNext: rows.length > PERSON_DISCOVERY_PAGE_LIMIT, nextCursor }) }) });
}
...
async function linkableContacts(database: D1Database, workspaceId: string) {
  ...
  return Object.freeze(rows.filter((row) => opaque(row.id) && positive(Number(row.revision)) && boundedText(row.display_name, 160)).map((row) => Object.freeze({ contactId: row.id, contactRevision: Number(row.revision), label: row.display_name })));
}

async function history(database: D1Database, workspaceId: string, subject: string, prospectId: string, now = Date.now()) {
  ...
  return Object.freeze({
    ...
    relevance: relevance.results.filter((row) => opaque(row.id) && opaque(row.prospect_id) && opaque(row.contact_id) && opaque(row.decision_id) && positive(Number(row.contact_revision)) && boundedText(row.display_name, 160) && boundedText(row.role_title, 160)).map((row) => Object.freeze({ relevanceId: row.id, prospectId: row.prospect_id, contactId: row.contact_id, contactRevision: Number(row.contact_revision), contactLabel: row.display_name, decisionId: row.decision_id, roleTitle: row.role_title, current: true as const, verificationChannels: Object.freeze(["email", "phone"] as const) })),
