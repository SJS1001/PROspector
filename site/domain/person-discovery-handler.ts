...
async function readProjection(request: Request, dependencies: PersonDiscoveryHandlerDependencies, workspaceId: string, subject: string) {
  ...
  const approved = (await Promise.all(approvedRows.filter((row) => opaque(row.id) && positive(row.revision)).map(async (row) => {
    ...
    return authority && authority.prospectRevision === Number(row.revision) ? Object.freeze({ prospectId: row.id, prospectRevision: Number(row.revision), knownPerson: Number(row.known_person) === 1 }) : null;
...
function sanitize(value: unknown) { if (!record(value) || !["accepted", "blocked", "conflict"].includes(String(value.kind))) return Object.freeze({ kind: "blocked", reason: "invalid_service_result" }); if (value.kind === "accepted") { if (value.replayed !== true && value.replayed !== false) return Object.freeze({ kind: "blocked", reason: "invalid_service_result" }); if (!record(value.run) && !record(value.decision) && !record(value.intent)) return Object.freeze({ kind: "blocked", reason: "invalid_service_result" }); return Object.freeze({ kind: "accepted", replayed: value.replayed }); } return typeof value.reason === "string" && value.reason.length > 0 && value.reason.length <= 200 ? Object.freeze({ kind: value.kind, reason: value.reason }) : Object.freeze({ kind: "blocked", reason: "invalid_service_result" }); }
function commandStatus(value: unknown) { const safe = sanitize(value); return safe.kind === "accepted" ? 200 : 409; }
