...
export async function persistQualificationAssessment(database:D1Database, principal:Principal, input:{candidateId:string; now?:number}) {
  ...
  if(priorInactive?.state==="disqualified"&&priorInactive.decision_at){
  ...
  const decisionAt=Number(priorInactive?.decision_at??0);
  ...
  const reentryKind=priorInactive?.state==="disqualified"&&sourcedDisproof?"hard_gate_disproved":priorInactive&&["rejected","deferred"].includes(priorInactive.state)&&(materialSignal||Number(priorInactive.ends_at)<=now)?(materialSignal?"material_signal":"review_due"):null;
  ...
  if(priorInactive?.state==="disqualified"&&!reentryKind) throw fail("Application-validated sourced disproof is required to reopen a disqualified Prospect");
