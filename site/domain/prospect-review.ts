...
export async function persistQualificationAssessment(database:D1Database, principal:Principal, input:{candidateId:string; now?:number}) {
  ...
  // The persisted lifecycle names an owner-disqualified Prospect `rejected`;
  // qualification hard-gate failures never create a Prospect. Consequently a
  // sourced-disproof marker is valid only against that exact rejected owner
  // decision, never a deferred row or an unpersisted assessment outcome.
  if(priorInactive?.state==="rejected"&&priorInactive.decision==="reject"&&priorInactive.decision_at){
  ...
  const reentryKind=priorInactive?.state==="rejected"&&sourcedDisproof?"hard_gate_disproved":priorInactive&&(materialSignal||priorInactive.ends_at<=now)?(materialSignal?"material_signal":"review_due"):null;
  const reentrySignal=reentryKind==="hard_gate_disproved"?sourcedDisproof:materialSignal;
