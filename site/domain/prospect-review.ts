...
export async function persistQualificationAssessment(database:D1Database, principal:Principal, input:{candidateId:string; now?:number}) {
  ...
  const priorInactive=await database.prepare("SELECT p.id,p.assessment_id,p.state,c.id cooldown_id,c.starts_at,c.ends_at,c.status,d.id decision_id,d.decision,d.review_at,d.decision_digest,d.created_at decision_at FROM profile_prospects p JOIN prospecting_candidates prior_candidate ON prior_candidate.id=p.candidate_id AND prior_candidate.workspace_id=p.workspace_id JOIN prospect_review_decisions d ON d.prospect_id=p.id AND d.workspace_id=p.workspace_id AND d.assessment_id=p.assessment_id AND ((p.state='rejected' AND d.decision='reject') OR (p.state='deferred' AND d.decision='defer')) LEFT JOIN prospect_cooldowns c ON c.workspace_id=p.workspace_id AND c.prospect_id=p.id AND c.review_decision_id=d.id AND c.assessment_id=p.assessment_id AND c.status='active' AND c.starts_at=d.created_at AND c.created_at=d.created_at AND ((d.decision='reject' AND c.ends_at=d.created_at+7776000000) OR (d.decision='defer' AND c.ends_at=d.review_at)) WHERE p.workspace_id=? AND p.profile_id=? AND p.offer_id=? AND (p.fingerprint=? OR (json_extract(prior_candidate.candidate_json,'$.accountId')=? AND json_extract(prior_candidate.candidate_json,'$.targetId')=?)) AND p.active=0 ORDER BY p.updated_at DESC,p.id DESC,d.created_at DESC,d.id DESC LIMIT 1").bind(workspace.id,candidate.profile_id,candidate.offer_id,prospectFingerprint,accountId,targetId).first<PriorInactive>();
  const priorDisqualified=!priorInactive?await database.prepare("SELECT a.id,a.assessment_digest,a.created_at disqualified_at FROM qualification_assessments a JOIN prospecting_candidates prior_candidate ON prior_candidate.id=a.candidate_id AND prior_candidate.workspace_id=a.workspace_id WHERE a.workspace_id=? AND prior_candidate.profile_id=? AND prior_candidate.offer_id=? AND (prior_candidate.fingerprint=? OR (json_extract(prior_candidate.candidate_json,'$.accountId')=? AND json_extract(prior_candidate.candidate_json,'$.targetId')=?)) AND a.outcome='Disqualified' ORDER BY a.created_at DESC,a.id DESC LIMIT 1").bind(workspace.id,candidate.profile_id,candidate.offer_id,prospectFingerprint,accountId,targetId).first<PriorDisqualified>():null;
  ...
  // A hard-gate failure is a durable assessment lifecycle even though it never
  // creates a reviewable Prospect. Sourced disproof must bind to that exact
  // tenant-scoped assessment and immutable digest.
  if(priorDisqualified){
    const signalId=await readValidatedSourcedDisproofSignalId(database,{workspaceId:workspace.id,profileId:candidate.profile_id,candidateId:candidate.id,candidateDigest:candidate.candidate_digest,fingerprint:prospectFingerprint,priorAssessmentId:priorDisqualified.id,priorAssessmentDigest:priorDisqualified.assessment_digest,disqualifiedAt:Number(priorDisqualified.disqualified_at)});
    if(signalId)sourcedDisproof=await database.prepare("SELECT ps.id,ps.source_lineage_id,ps.signal_digest,ps.material,pl.source_tier,pl.independence_group,pl.occurred_at,pl.retrieved_at,pl.lineage_digest,ps.signal_json FROM prospecting_signals ps JOIN prospecting_source_lineage pl ON pl.id=ps.source_lineage_id AND pl.workspace_id=ps.workspace_id WHERE ps.id=? AND ps.workspace_id=? AND ps.profile_id=? LIMIT 1").bind(signalId,workspace.id,candidate.profile_id).first<SignalRow>()??undefined;
  }
  ...
  const decisionAt=priorInactive?priorInactive.decision_at:0;
  const materialSignal=signalRows.results.filter(x=>Boolean(x.material)&&Number(x.occurred_at)>decisionAt&&Number(x.retrieved_at)>decisionAt).sort((a,b)=>Number(b.retrieved_at)-Number(a.retrieved_at)||a.id.localeCompare(b.id))[0];
  if(priorInactive&&!validBoundCooldown(priorInactive))throw fail("A correctly bound Prospect cooldown is required for re-entry");
  const cooldownActive=Boolean(priorInactive&&priorInactive.ends_at>now);
  const reentryKind=priorInactive&&(materialSignal||priorInactive.ends_at<=now)?(materialSignal?"material_signal":"review_due"):null;
  const reentrySignal=materialSignal;
  if(cooldownActive&&!reentryKind) throw fail("Prospect cooldown remains active; a Material Signal is required for re-entry");
  if(priorInactive?.state==="rejected"&&!reentryKind) throw fail("Rejected Prospect cooldown remains active; re-entry requires expiry or a Material Signal");
  if(priorDisqualified&&evaluation.outcome==="Passed"&&!sourcedDisproof)throw fail("Disqualified assessment re-entry requires application-validated sourced disproof");
  ...
  try { await database.batch([
    ...
    ...(priorInactive&&reentryKind&&evaluation.outcome==="Passed"?[database.prepare("INSERT INTO authority_commands (id,workspace_id,created_at,updated_at,revision,command_type,idempotency_key,operation_digest,expected_revision,subject_type,subject_id,status) VALUES (?,?,?,?,1,'prospect.reentry',?,?,1,'profile_prospect',?,'accepted')").bind(reentryCommandId,workspace.id,now,now,`reentry:${assessmentDigest}`,await sha256(stable({priorProspectId:priorInactive.id,assessmentDigest,reentryKind,signalId:reentrySignal?.id??null})),priorInactive.id),database.prepare("INSERT INTO audit_events (id,workspace_id,actor_type,actor_id,action,subject_type,subject_id,detail_json,created_at) VALUES (?,?,'system','prospect-assessment-service','prospect.reentry','profile_prospect',?,?,?)").bind(reentryAuditId,workspace.id,priorInactive.id,stable({reentryKind,signalId:reentrySignal?.id??null,assessmentId,reenteredProspectId:prospectId}),now),database.prepare("INSERT INTO prospect_reentry_events (id,workspace_id,prospect_id,cooldown_id,signal_id,prior_assessment_id,event_kind,event_json,event_digest,authority_command_id,audit_event_id,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").bind(reentryId,workspace.id,priorInactive.id,priorInactive.cooldown_id,reentrySignal?.id??null,priorInactive.assessment_id,reentryKind,stable({assessmentId,candidateId:candidate.id,candidateDigest:candidate.candidate_digest,priorProspectId:priorInactive.id,reenteredProspectId:prospectId,signalId:reentrySignal?.id??null}),await sha256(stable({priorProspectId:priorInactive.id,assessmentDigest,reentryKind,signalId:reentrySignal?.id??null})),reentryCommandId,reentryAuditId,now)]:[]),
    ...(priorDisqualified&&sourcedDisproof&&evaluation.outcome==="Passed"?[database.prepare("INSERT INTO authority_commands (id,workspace_id,created_at,updated_at,revision,command_type,idempotency_key,operation_digest,expected_revision,subject_type,subject_id,status) VALUES (?,?,?,?,1,'prospect.reentry',?,?,1,'qualification_assessment',?,'accepted')").bind(reentryCommandId,workspace.id,now,now,`reentry:${assessmentDigest}`,await sha256(stable({priorAssessmentId:priorDisqualified.id,priorAssessmentDigest:priorDisqualified.assessment_digest,assessmentDigest,reentryKind:"hard_gate_disproved",signalId:sourcedDisproof.id})),priorDisqualified.id),database.prepare("INSERT INTO audit_events (id,workspace_id,actor_type,actor_id,action,subject_type,subject_id,detail_json,created_at) VALUES (?,?,'system','prospect-assessment-service','prospect.reentry','qualification_assessment',?,?,?)").bind(reentryAuditId,workspace.id,priorDisqualified.id,stable({reentryKind:"hard_gate_disproved",signalId:sourcedDisproof.id,assessmentId,reenteredProspectId:prospectId}),now)]:[]),
...
export async function decideQualifiedProspect(database:D1Database, principal:Principal, input:{prospectId:string; assessmentId?:string; decision:"approve"|"reject"|"defer"; reason?:string; reviewAt?:number; expectedRevision:number; idempotencyKey:string; now?:number}) {
  ...
  const workspace=await workspaceFor(database,principal);
  const prior=await database.prepare("SELECT id,prospect_id,assessment_id,operation_digest FROM prospect_review_decisions WHERE workspace_id=? AND idempotency_key=? LIMIT 1").bind(workspace.id,input.idempotencyKey).first<{id:string;prospect_id:string;assessment_id:string;operation_digest:string}>();
  if(prior){
    const replayOperation=await reviewOperationDigest({prospectId:input.prospectId,assessmentId:input.assessmentId??prior.assessment_id,decision:input.decision,reason,reviewAt:input.reviewAt??null,revision:input.expectedRevision});
    if(prior.prospect_id!==input.prospectId||prior.assessment_id!==(input.assessmentId??prior.assessment_id)||prior.operation_digest!==replayOperation)throw fail("Idempotency key conflicts with a different review command");
    return {decisionId:prior.id,replayed:true};
  }
  ...
  const operation=await reviewOperationDigest({prospectId:prospect.id,assessmentId:prospect.assessment_id,decision:input.decision,reason,reviewAt:input.reviewAt??null,revision:prospect.revision});
...
type PriorInactive={id:string;assessment_id:string;state:"rejected"|"deferred";cooldown_id:string|null;starts_at:number|null;ends_at:number|null;status:string|null;decision_id:string;decision:"reject"|"defer";review_at:number|null;decision_digest:string;decision_at:number};
type PriorDisqualified={id:string;assessment_digest:string;disqualified_at:number};
type BoundCooldown=PriorInactive&{cooldown_id:string;starts_at:number;ends_at:number;status:"active"};
function validBoundCooldown(prior:PriorInactive):prior is BoundCooldown{return typeof prior.cooldown_id==="string"&&prior.cooldown_id.length>0&&prior.status==="active"&&typeof prior.starts_at==="number"&&Number.isSafeInteger(prior.starts_at)&&prior.starts_at===prior.decision_at&&typeof prior.ends_at==="number"&&Number.isSafeInteger(prior.ends_at)&&prior.ends_at>prior.starts_at&&(prior.decision==="reject"&&prior.state==="rejected"&&prior.ends_at===prior.decision_at+90*86400000||prior.decision==="defer"&&prior.state==="deferred"&&typeof prior.review_at==="number"&&Number.isSafeInteger(prior.review_at)&&prior.ends_at===prior.review_at);}
...
function reviewOperationDigest(input:{prospectId:string;assessmentId:string;decision:"approve"|"reject"|"defer";reason:string;reviewAt:number|null;revision:number}){return sha256(stable({action:"prospect.review",...input}));}
function text(x:unknown){return typeof x==="string"?x:"";} function number(x:unknown){return Number.isInteger(x)?x:0;} function validKey(x:string){return typeof x==="string"&&/^[a-f0-9-]{20,80}$/i.test(x);}
