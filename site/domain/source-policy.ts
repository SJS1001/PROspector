...
type DisproofCandidate=Readonly<{id:string;workspace_id:string;profile_id:string;run_id:string;submission_id:string;candidate_digest:string;fingerprint:string}>;
...
export async function appendSourcedDisproofValidationMarker(database:D1Database,input:{
  ...
  const candidate=await database.prepare("SELECT id,workspace_id,profile_id,run_id,submission_id,candidate_digest,fingerprint FROM prospecting_candidates WHERE id=? AND workspace_id=? AND profile_id=? LIMIT 1").bind(input.candidateId,input.workspaceId,input.profileId).first<DisproofCandidate>();
  if(!candidate||!validDigest(candidate.candidate_digest)||!validDigest(candidate.fingerprint))throw fail();
  const prior=await latestRejectedProspect(database,input.workspaceId,candidate.fingerprint);
  if(!prior)throw fail();
  ...
  const validation={
    ...
    candidateId:candidate.id,candidateDigest:candidate.candidate_digest,priorProspectId:prior.id,
...
export async function readValidatedSourcedDisproofSignalId(database:D1Database,input:{
  ...
  for(const row of rows){
    try{
      ...
      if(marker.schema!=="prospect-reentry-disproof-validation/v1"||marker.verdict!=="sourced_disproof"||marker.validatedBy!=="application"||marker.validationRule!=="owner-rejection-material-evidence/v1"||marker.workspaceId!==input.workspaceId||marker.profileId!==input.profileId||marker.candidateId!==input.candidateId||marker.candidateDigest!==input.candidateDigest||marker.priorProspectId!==input.priorProspectId||marker.priorAssessmentId!==input.priorAssessmentId||marker.decisionAt!==input.decisionAt)continue;
      const prior=await latestRejectedProspect(database,input.workspaceId,input.fingerprint);if(!prior||prior.id!==marker.priorProspectId||prior.assessment_id!==marker.priorAssessmentId||prior.decision_id!==marker.reviewDecisionId||prior.decision_digest!==marker.reviewDecisionDigest||prior.decision_at!==marker.decisionAt)continue;
...
async function latestRejectedProspect(database:D1Database,workspaceId:string,fingerprint:string):Promise<RejectedProspect|null>{
  return database.prepare("SELECT p.id,p.assessment_id,d.id decision_id,d.decision_digest,d.created_at decision_at FROM profile_prospects p JOIN prospect_review_decisions d ON d.prospect_id=p.id AND d.workspace_id=p.workspace_id AND d.assessment_id=p.assessment_id WHERE p.workspace_id=? AND p.fingerprint=? AND p.state='rejected' AND p.active=0 AND d.decision='reject' ORDER BY d.created_at DESC,d.id DESC LIMIT 1").bind(workspaceId,fingerprint).first<RejectedProspect>();
}
