...
export type SourcedDisproofSelection=Readonly<{signalFingerprint:string;validationRule:"disqualified-assessment-material-evidence/v1"}>;
...
type DisqualifiedAssessment=Readonly<{id:string;candidate_id:string;candidate_digest:string;assessment_digest:string;disqualified_at:number}>;
...
export async function appendSourcedDisproofValidationMarker(database:D1Database,input:{
  ...
  if(!validId(input.workspaceId)||!validId(input.profileId)||!validId(input.candidateId)||!validId(input.runId)||!validId(input.submissionId)||!Number.isSafeInteger(input.now)||input.now<=0||!input.selection||input.selection.validationRule!=="disqualified-assessment-material-evidence/v1"||!validDigest(input.selection.signalFingerprint))throw fail();
  ...
  if(!identity||candidate.fingerprint!==identity.fingerprint)throw fail();
  const prior=await latestDisqualifiedAssessment(database,identity);
  if(!prior)throw fail();
  ...
  if(!signal||!await validDisproofSourceFact(signal)||Number(signal.occurred_at)<=prior.disqualified_at||Number(signal.retrieved_at)<=prior.disqualified_at)throw fail();
  const validation={
    schema:"prospect-reentry-disproof-validation/v2",verdict:"sourced_disproof",validatedBy:"application",anchorKind:"qualification_assessment_disqualified",
    validationRule:input.selection.validationRule,workspaceId:input.workspaceId,profileId:input.profileId,
    candidateId:candidate.id,candidateDigest:candidate.candidate_digest,prospectFingerprint:identity.fingerprint,
    priorCandidateId:prior.candidate_id,priorCandidateDigest:prior.candidate_digest,priorAssessmentId:prior.id,
    priorAssessmentDigest:prior.assessment_digest,disqualifiedAt:prior.disqualified_at,sourceLineageId:signal.source_lineage_id,sourceLineageDigest:signal.lineage_digest,
 ...
 * Disqualified assessment anchor, signal, lineage, workspace, and timestamps. */
export async function readValidatedSourcedDisproofSignalId(database:D1Database,input:{
  ...
  priorAssessmentId:string; priorAssessmentDigest:string; disqualifiedAt:number;
}):Promise<string|null>{
  if(!validId(input.workspaceId)||!validId(input.profileId)||!validId(input.candidateId)||!validDigest(input.candidateDigest)||!validDigest(input.fingerprint)||!validId(input.priorAssessmentId)||!validDigest(input.priorAssessmentDigest)||!Number.isSafeInteger(input.disqualifiedAt)||input.disqualifiedAt<=0)return null;
  const candidate=await database.prepare("SELECT id,workspace_id,profile_id,offer_id,run_id,submission_id,candidate_digest,fingerprint,candidate_json FROM prospecting_candidates WHERE id=? AND workspace_id=? AND profile_id=? LIMIT 1").bind(input.candidateId,input.workspaceId,input.profileId).first<DisproofCandidate>();
  const identity=candidate&&await prospectIdentity(candidate);
  if(!candidate||candidate.candidate_digest!==input.candidateDigest||candidate.fingerprint!==input.fingerprint||!identity||identity.fingerprint!==input.fingerprint)return null;
  const rows=(await database.prepare("SELECT id,lineage_json,lineage_digest FROM prospecting_source_lineage WHERE workspace_id=? AND json_extract(lineage_json,'$.schema')='prospect-reentry-disproof-validation/v2' ORDER BY created_at DESC,id DESC").bind(input.workspaceId).all<{id:string;lineage_json:string;lineage_digest:string}>()).results;
  for(const row of rows){
    try{
      ...
      if(marker.schema!=="prospect-reentry-disproof-validation/v2"||marker.verdict!=="sourced_disproof"||marker.validatedBy!=="application"||marker.anchorKind!=="qualification_assessment_disqualified"||marker.validationRule!=="disqualified-assessment-material-evidence/v1"||marker.workspaceId!==input.workspaceId||marker.profileId!==input.profileId||marker.candidateId!==input.candidateId||marker.candidateDigest!==input.candidateDigest||marker.prospectFingerprint!==input.fingerprint||marker.priorAssessmentId!==input.priorAssessmentId||marker.priorAssessmentDigest!==input.priorAssessmentDigest||marker.disqualifiedAt!==input.disqualifiedAt)continue;
      const prior=await latestDisqualifiedAssessment(database,identity);if(!prior||prior.id!==marker.priorAssessmentId||prior.assessment_digest!==marker.priorAssessmentDigest||prior.candidate_id!==marker.priorCandidateId||prior.candidate_digest!==marker.priorCandidateDigest||prior.disqualified_at!==marker.disqualifiedAt)continue;
      const signal=await database.prepare("SELECT ps.id,ps.run_id,ps.submission_id,ps.source_lineage_id,ps.signal_json,ps.signal_digest,ps.material,pl.lineage_json,pl.lineage_digest,pl.source_url,pl.publisher_identity,pl.underlying_origin_identity,pl.independence_group,pl.source_tier,pl.occurred_at,pl.retrieved_at,json_extract(pl.lineage_json,'$.observationFingerprint') fingerprint FROM prospecting_signals ps JOIN prospecting_source_lineage pl ON pl.id=ps.source_lineage_id AND pl.workspace_id=ps.workspace_id WHERE ps.id=? AND ps.workspace_id=? AND ps.profile_id=? AND ps.source_lineage_id=? LIMIT 1").bind(marker.signalId,input.workspaceId,input.profileId,marker.sourceLineageId).first<DisproofSignal>();
      if(!signal||!await validDisproofSourceFact(signal)||signal.signal_digest!==marker.signalDigest||signal.lineage_digest!==marker.sourceLineageDigest||Number(signal.occurred_at)!==marker.observedAt||Number(signal.retrieved_at)!==marker.retrievedAt||Number(signal.occurred_at)<=prior.disqualified_at||Number(signal.retrieved_at)<=prior.disqualified_at)continue;
      return signal.id;
    }catch{continue;}
  }
  return null;
...
async function latestDisqualifiedAssessment(database:D1Database,identity:ProspectIdentity):Promise<DisqualifiedAssessment|null>{
  return database.prepare("SELECT a.id,a.candidate_id,c.candidate_digest,a.assessment_digest,a.created_at disqualified_at FROM qualification_assessments a JOIN prospecting_candidates c ON c.id=a.candidate_id AND c.workspace_id=a.workspace_id WHERE a.workspace_id=? AND c.profile_id=? AND c.offer_id=? AND (c.fingerprint=? OR (json_extract(c.candidate_json,'$.accountId')=? AND json_extract(c.candidate_json,'$.targetId')=?)) AND a.outcome='Disqualified' ORDER BY a.created_at DESC,a.id DESC LIMIT 1").bind(identity.workspaceId,identity.profileId,identity.offerId,identity.fingerprint,identity.accountId,identity.targetId).first<DisqualifiedAssessment>();
}
