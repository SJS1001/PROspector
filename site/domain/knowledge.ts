...
async function versionById(database: D1Database, workspaceId: string, id: string, predecessorId: string | null = null) { const row = await database.prepare("SELECT id, proposal_id, decision_id, scope_type, scope_id, kind, value_json, value_digest FROM knowledge_versions WHERE id = ? AND workspace_id = ? LIMIT 1").bind(id, workspaceId).first<{ id: string; proposal_id: string; decision_id: string; scope_type: string; scope_id: string; kind: string; value_json: string; value_digest: string }>(); if (!row) throw new KnowledgeConflictError("Knowledge version is unavailable"); const decision = await database.prepare("SELECT decision FROM proposal_decisions WHERE id = ? AND workspace_id = ? LIMIT 1").bind(row.decision_id, workspaceId).first<{ decision: string }>(); const locator = await destinationLocator(database, workspaceId, row.scope_type, row.scope_id); return { id: row.id, type: "knowledge_version", immutable: true, predecessorId, successorLineage: { decision: decision?.decision }, destination: { scopeType: publicScopeType(row.scope_type), id: row.scope_id, locator }, kind: row.kind, value: JSON.parse(row.value_json), digest: row.value_digest }; }
