...
type Destination = { scopeType: "company" | "product" | "market_play" | "customer_profile" | "offer"; locator: string; companyId?: string };
...
export async function createKnowledgeProposal(database: D1Database, principal: InterviewPrincipal, input: ProposalInput) {
  ...
  try {
    await database.batch([
      ...
      ...(origin === "quarantined_upload" ? [database.prepare("INSERT INTO source_custody (id, workspace_id, created_at, updated_at, revision, source_id, object_reference, quarantine_status, scan_status, object_digest) VALUES (?, ?, ?, ?, 1, ?, ?, 'quarantined', 'not_scanned', ?)").bind(v7(), workspace.id, now, now, sourceId, provenance.reference, sourceDigest)] : []),
...
async function resolveDestination(database: D1Database, workspaceId: string, destination: Destination) { const mapping = destination.scopeType === "company" ? { table: "companies", scope: "company" } : destination.scopeType === "product" ? { table: "products", scope: "product" } : destination.scopeType === "market_play" ? { table: "market_plays", scope: "play" } : destination.scopeType === "customer_profile" ? { table: "customer_profiles", scope: "profile" } : { table: "offers", scope: "offer" }; const row = await database.prepare(`SELECT id FROM ${mapping.table} WHERE workspace_id = ? AND name = ? LIMIT 1`).bind(workspaceId, bounded(destination.locator, 160, "destination locator")).first<{ id: string }>(); if (!row) throw new KnowledgeConflictError("Destination is outside the authorized commercial hierarchy"); return { id: row.id, scopeType: mapping.scope }; }
