...
export default function LocalDemo() {
  ...
  async function bootstrap() {
    const read = await fetch("/api/interview", { cache: "no-store" });
    if (!read.ok) return setState("Local demo unavailable.");
    const csrf = read.headers.get("set-cookie")?.match(/prospector_csrf=([^;]+)/)?.[1];
    const response = await fetch("/api/interview", { method: "POST", headers: { "content-type": "application/json", "x-prospector-intent": "interview-mutation", ...(csrf ? { cookie: `prospector_csrf=${csrf}` } : {}) }, body: JSON.stringify({ action: "bootstrap" }) });
