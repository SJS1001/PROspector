...
function dependencies(request: Request): ProspectingHandlerDependencies { const b = env as unknown as { DB?: D1Database; OWNER_SUBJECT_PEPPER?: string; PILOT_OWNER_EMAIL?: string; TRUSTED_IDENTITY_PROVIDER?: string; LOCAL_DEMO?: string; CLOUDFLARE_ACCESS_ISSUER?: string; CLOUDFLARE_ACCESS_AUDIENCE?: string }; if (!b.DB || !b.OWNER_SUBJECT_PEPPER || !b.PILOT_OWNER_EMAIL) throw new Error("Secure prospecting bindings are unavailable"); return { database: b.DB, subjectPepper: b.OWNER_SUBJECT_PEPPER, pilotOwnerEmail: b.PILOT_OWNER_EMAIL, csrfCookieMode: isLocalDemoRequest(request, b) ? "local-demo" : "secure", getIdentity: async () => runtimeIdentity(request, b) }; }
