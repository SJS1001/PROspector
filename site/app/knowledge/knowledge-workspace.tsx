...
export function KnowledgeWorkspace({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  const disabled = mutating ? <p className="saved" role="status">Saving the reviewed authority snapshot… Other actions are temporarily disabled.</p> : null;
  return <section className="knowledge-workspace" aria-live="polite">
    ...
    <fieldset className="knowledge-action-boundary" disabled={mutating}><legend className="sr-only">Authoritative knowledge actions</legend>
      {view === "Commercial Model" && <CommercialModelView projection={commercial} operationKey="commercial" onCreateDraft={(command) => void dispatch("create_hierarchy_draft", `draft:${command.type}:${command.parentId}:${command.name}`, draftPayload(command))} onProposeChange={(node) => void dispatch("propose_owner_edit", `owner-edit:${node.id}`, proposalPayload(node))} />}
      {view === "Interview" && <InterviewPane state={interview} dispatch={dispatch} />}
      {view === "Knowledge Library" && <KnowledgeLibraryView items={library} operationKey="library" onIntake={(command) => void dispatch(command.action, `${command.action}:${command.source.reference}:${command.text}`, intakePayload(command))} onReviewProposal={(command) => void dispatch("review_knowledge_proposal", `review:${command.proposalId}:${command.decision}`, reviewPayload(command))} onProposeChange={(item) => void dispatch("propose_owner_edit", `owner-edit:${item.id}`, proposalPayload(item))} />}
      {view === "Drift & Replacements" && <DriftReplacementsView drift={drift} candidates={replacements} operationKey="replacement" onCreateCandidate={(command) => void dispatch("create_replacement_candidate", `candidate:${command.proposedVersionId}`, candidatePayload(command))} onActivateReplacement={(command) => void dispatch("activate_replacement", `activate:${command.candidateId}`, activationPayload(command))} />}
    </fieldset>
  </section>;
}

function InterviewPane({ state, dispatch }: { state: InterviewState; dispatch: (action: string, logicalKey: string, fields: Record<string, unknown>) => Promise<void> }) {
...
function proposalPayload(source: CommercialHierarchyNode | KnowledgeItemProjection) { const destination = "parentId" in source ? { scopeType: source.type, locator: source.name } : { scopeType: source.destination.scopeType, locator: source.destination.id }; return { destination, kind: "owner_edit", text: "Owner requested a proposed change from the displayed authoritative scope.", source: { reference: `owner-ui:${source.id}`, custody: "owner-entered workspace change", retrievedAt: Date.now() }, privacy: "private", license: { use: "owner review" }, reuseEligibility: "owner_edit" }; }
