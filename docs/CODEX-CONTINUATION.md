...
phone-effect authority, and none executes a Phase 6 plan. The next safe unit is
a read-only Phase 7 preparation-authority audit against its checked context,
research, UI, validation, and plan dependency matrix. Do not implement Phase 7
or create its missing contract tests unless a bounded local-only preparation
lane is durably authorized with the same zero-effect safeguards. Do not
