...
- Phase 6 preparation and 13 checked plans are committed; execution has not started. The preparation lane includes a static provider/effect guard, an isolated synthetic approval/suppression state machine, and a canonical synthetic Package/Message artifact plus invalidation contract. Runtime code imports none of these preparation modules.
...
complete local preparation; they do not execute a Phase 6 plan. The next safe
unit may model a pure provider-neutral final-dispatch recheck/lease decision,
but must remain synthetic, reject-only, outside runtime composition, and
literal zero-effect. Do not access the original target or provision a new one.
