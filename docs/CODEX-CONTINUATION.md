...
suppression-retention manifest across delete/import/export/archive/restore is
also complete. Collectively they grant
...
model one pure synthetic cross-contract Phase 6 invariant bundle that binds
the already-verified preparation decisions by ID and digest only, but it must
remain outside runtime composition, perform no persistence or effect, accept
no real identity data, and retain literal zero effects. Do not
