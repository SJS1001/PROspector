...
- A selected answer prepares its source, excerpt, Proposed Knowledge, proposal audit, answer, question transition, session transition, and answer audit in one D1 batch led by an exact Explore authority guard. Losing the immutable selection or the Draft/Active Play lifecycle before that batch leaves every prepared row unapplied.
