...
This lane was first implemented and validated in a prior cloud session whose
container was reclaimed before its commit could be pushed. The results below
record which machine produced each one; nothing here is carried over as an
unverified claim.

Re-verified in this recovery worktree (macOS, Node.js `v24.16.0`), from `site/`:

- `node --test --test-concurrency=1 tests/synthetic-enrichment-reconciliation-decision.test.mjs` — 13/13 pass.

Recorded from the original session (Linux, Node.js `v22.22.2`), from `site/`:

- the same focused suite — 13/13 pass.
...
- `node scripts/run-test-suite.mjs tests/contact-eligibility.test.mjs tests/identity-resolution.test.mjs tests/synthetic-enrichment-prerequisite-plan.test.mjs` — all pass.
- `npm run lint` — clean. `npx tsc --noEmit` — zero errors in this module.

Known pre-existing failure, not caused by this lane: `tests/drift-replacement.test.mjs`
fails 3 of 6 cases (`not ok 4`, `5`, `6`) with `Commercial workspace is unavailable`
/ `knowledge_conflict` at `site/domain/knowledge.ts:435`. This was confirmed by
running that file at base commit `5c3440e11dc32beaed7dfc3d6e1bf11aafd3945c` with
this lane's files absent, which reproduced the identical three failures. The
canonical `npm test` therefore stops at that file; the remaining suite files were
not run to completion in either session.
