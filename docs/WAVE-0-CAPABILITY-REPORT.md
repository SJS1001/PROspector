...
Site source commit: `d6b3b6196cfe88e1141c9e5e8dd42a4cefb0dcdd`
...
Version 10 is the accepted hosted release. It was built, tested, pushed to the
existing Sites source repository, saved, and deployed from the exact source
commit above. The remaining Wave 0 blocker is the real second-principal
isolation checkpoint, not deployment or R2 durability.
...
| Exact-source release | Pass | Site source `d6b3b61…` was pushed, packaged, saved as version 10, and deployed successfully with environment revision 2. |
| Local build | Pass | Lint, production build, 15/15 persistence/security tests, and React Doctor 100/100 pass. |
| Authenticated hosted bindings | Pass | The owner-only application reads D1-backed evidence and the fixed R2 proof completed write, read, digest, delete, and absence through the hosted adapter. |
| Hosted R2 lifecycle | Pass | Version 10 fixed proof completed all five steps and remained Proven after reload at `2026-07-30T15:49:48.528Z` with opaque audit reference `ae_cap_99ea450032f22a841ffbafbf`. |
...
| Server secrets | Partial | Sites environment revision 2 contains the required server-only owner and subject-pepper secrets; values are not returned, displayed, or committed. Coordinated rotation and identity migration remain unproven. |
| Mutation/CSRF controls | Partial | Local handler and harness tests prove trusted injected identity, spoofed-header denial, exact Origin, Fetch Metadata, JSON and byte bounds, one-time owner-bound CSRF, replay denial, and cross-owner rejection. Version 10 transports CSRF only in a short-lived `HttpOnly`, `Secure`, `SameSite=Strict` cookie. A fresh hosted proof logged the Cookie as redacted and no custom CSRF header. Authenticated hosted negative cases still require operator-session execution. |
...
2. A decision and proof path for scheduled execution and Runner callbacks on
   ...
   ports.
3. A controlled second signed-in principal for hosted cross-principal isolation proof; do not enable an application invitation.
